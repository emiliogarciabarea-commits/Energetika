...
def generar_pdf(df_detalle, df_ranking, df_consumos, df_precios_ganadora, nombre_cliente, direccion_cliente, compania_actual_manual, mostrar_nombres):
    try:
        ...
        ahorro_total_periodo = ranking_real.iloc[:, 1].max() # Usamos el valor real para el cálculo
        ...
        pdf.ln(8)
        pdf.set_font('Arial', 'B', 10); pdf.set_text_color(20, 50, 100); pdf.cell(0, 10, "1. COMPARATIVA DE COSTES Y AHORRO MENSUAL", ln=True)
        pdf.set_x(25); pdf.set_fill_color(210, 225, 240); pdf.set_font('Arial', 'B', 8)
        ...
        pdf.ln(10) 
        pdf.set_x(20); pdf.set_font('Arial', '', 11); pdf.multi_cell(170, 7, f"Tras analizar el mercado, la opción más eficiente para su suministro es:", align='C')
        pdf.set_x(20); pdf.set_font('Arial', 'B', 14); pdf.set_text_color(20, 50, 100); pdf.cell(170, 12, f"{str(nombre_ganadora).upper()}", border='TLR', ln=True, align='C')
        pdf.set_x(20); pdf.set_font('Arial', 'B', 12); pdf.set_text_color(34, 139, 34); pdf.cell(170, 12, f"AHORRO ANUAL ESTIMADO CON IVA: {round(ahorro_anual_con_iva, 2)} EUR / AÑO", border='BLR', ln=True, align='C')
        
        pdf.ln(15)
        url_wa = "https://wa.me/34614676150?text=Hola,%20me%20interesa%20contratar%20la%20tarifa%20ganadora"
        pdf.set_font('Arial', 'B', 10); pdf.set_text_color(20, 50, 100); pdf.cell(0, 5, "Haz clic en el icono para contratación directa:", ln=True, align='C')
        if os.path.exists("Whatsapp.png"):
            pdf.image("Whatsapp.png", x=100, y=pdf.get_y() + 5, w=15, link=url_wa)
