...
def generar_pdf(df_detalle, df_ranking, df_consumos, df_precios_ganadora, nombre_cliente, direccion_cliente, compania_actual_manual):
    ...
    try:
        ...
        pdf.set_x(35); pdf.set_fill_color(20, 50, 100); pdf.set_text_color(255); pdf.cell(80, 7, encode_text(" Compañía / Tarifa"), 1, 0, 'L', True); pdf.cell(60, 7, " Ahorro Total Detectado", 1, 1, 'C', True)
        pdf.set_text_color(0); pdf.set_font('Arial', '', 8)
        for _, row in ranking_ordenado.head(5).iterrows():
            pdf.set_x(35); pdf.cell(80, 7, f" {encode_text(row.iloc[0])}", 1); pdf.set_text_color(34, 139, 34); pdf.cell(60, 7, f" +{round(row.iloc[1], 2)} EUR", 1, 1, 'C'); pdf.set_text_color(0)
        ...
        pdf.multi_cell(170, 7, encode_text("Proyección de ahorro anual para las opciones más competitivas del mercado:"), align='C')
        pdf.ln(3)

        pdf.set_x(10); pdf.set_fill_color(245, 245, 245); pdf.set_font('Arial', 'B', 8)
        pdf.cell(60, 7, encode_text(" Compañía / Tarifa"), 1, 0, 'C', True)
        ...
        for _, row in ranking_ordenado.head(5).iterrows():
            ...
            pdf.set_x(10); pdf.cell(60, 7, f" {encode_text(nombre_cia)}", 1)
        ...
        pdf.ln(5) 
        pdf.set_x(20); pdf.set_font('Arial', '', 11); pdf.multi_cell(170, 7, encode_text(f"La opción más eficiente para su suministro es {nombre_ganadora}:"), align='C')
        pdf.set_x(20); pdf.set_font('Arial', 'B', 12); pdf.set_text_color(20, 50, 100); pdf.cell(170, 9, f"{encode_text(str(nombre_ganadora).upper())}", border='TLR', ln=True, align='C')
        ...
        pdf.cell(170, 8, encode_text(f"Con este cambio, dejarás de pagar {round(ahorro_5_anos, 2)} EUR extra en los próximos 5 años."), ln=True, align='C')
        ...
        pdf.cell(0, 4, encode_text("Escanea para contratación directa vía WhatsApp:"), ln=True, align='C')
        pdf.image(qr_path, x=92, w=25) 

        # Corregido: Obtener el output como string y codificarlo manualmente a latin-1 para Streamlit
