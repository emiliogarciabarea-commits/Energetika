...
class EnergetikaPDF(FPDF):
    def header(self):
        if os.path.exists("Logo_Energetika.png"):
            self.image("Logo_Energetika.png", 135, 10, 60)
...
def generar_pdf(df_detalle, df_ranking, df_consumos, df_precios_ganadora, nombre_cliente, direccion_cliente, compania_actual_manual, mostrar_nombres_reales):
    try:
        pdf = EnergetikaPDF()
        ...
        # Identificamos el nombre original de la ganadora para buscar en los otros DataFrames
        nombre_original_ganadora = ranking_ordenado.iloc[0, 0]
        
        # Lógica de Nomenclatura
        if not mostrar_nombres_reales:
            mapeo_nombres = {}
            for i, nombre in enumerate(ranking_ordenado.iloc[:, 0]):
                if i == 0:
                    mapeo_nombres[nombre] = "Tarifa Óptima Energetika"
                else:
                    mapeo_nombres[nombre] = f"Alternativa de Mercado {chr(64 + i)}" # A, B, C...
            
            # Aplicamos el mapeo al ranking que se mostrará
            ranking_ordenado.iloc[:, 0] = ranking_ordenado.iloc[:, 0].map(mapeo_nombres)
            nombre_ganadora_display = "Tarifa Óptima Energetika"
        else:
            nombre_ganadora_display = nombre_original_ganadora
        ...
        for fecha in lista_fechas:
            ...
            try:
                ...
                c_pro = mes_data[mes_data['Compañía/Tarifa'] == nombre_original_ganadora]['Coste (€)'].values[0]
        ...
        pdf.set_font('Arial', 'B', 10); pdf.set_text_color(20, 50, 100); pdf.cell(0, 10, f"4. DESGLOSE DE COSTES ESTIMADOS ({nombre_ganadora_display})", ln=True)
        ...
        pdf.ln(5) 
        pdf.set_x(20); pdf.set_font('Arial', '', 11); pdf.multi_cell(170, 7, f"La opción más eficiente para su suministro es {nombre_ganadora_display}:", align='C')
        pdf.set_x(20); pdf.set_font('Arial', 'B', 12); pdf.set_text_color(20, 50, 100); pdf.cell(170, 9, f"{str(nombre_ganadora_display).upper()}", border='TLR', ln=True, align='C')
        ...
        pdf.image("Whatsapp.png", x=100, y=pdf.get_y() + 6, w=13, link=url_wa)
        pdf.ln(25)
...
with c2: 
    ...
    # Selector de Nombres
    opcion_nombres = st.radio("Nombres de compañías en el PDF:", 
                              ("Nombres Reales", "Nomenclatura Energetika (Tarifa Óptima / Alternativas)"))

mostrar_nombres_reales = True if opcion_nombres == "Nombres Reales" else False
...
if archivo:
    try:
        ...
        if st.button("🚀 Generar PDF"):
            ...
            pdf_bytes = generar_pdf(df_det, df_ran, df_con, df_pre, nombre_cliente, direccion_cliente, compania_actual_manual, mostrar_nombres_reales)
            if pdf_bytes:
